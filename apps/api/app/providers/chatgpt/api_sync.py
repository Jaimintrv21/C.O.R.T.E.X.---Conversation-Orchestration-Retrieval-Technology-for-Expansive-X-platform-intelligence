"""ChatGPT API-log sync.

This syncs the user's own API usage logs or self-hosted logging stream, not
consumer chatgpt.com browser history. The supported path is BYO logging via the
...
class ChatGPTApiLogSyncProvider(BaseProvider):
    name = "ChatGPT API Logs"
    slug = "chatgpt-api-log"
    version = "1.0"
    ...
    async def sync(self, account: dict) -> AsyncIterator[CanonicalConversation]:
        ...
        for log in logs:
            for conversation in normalize_extension_conversations("chatgpt", log.get("conversations") or []):
                yield conversation

    def get_schema_version(self) -> str:
        return "chatgpt.api_log.v1"
